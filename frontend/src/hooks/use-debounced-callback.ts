 ...
 * @see {@link https://github.com/xnimorz/use-debounce/blob/master/src/useDebouncedCallback.ts | source}
 * @example
 *
...
>(func: T, wait: number = 0, options: Options = {}): DebouncedState<T> {
  ...
  const debounced = useMemo(() => {
    ...
    const shouldInvoke = (time: number) => {
      ...
      if (lastCallTime.current === null) return false;
